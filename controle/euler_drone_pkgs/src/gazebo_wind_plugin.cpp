...
#include "euler_helices_v2/gazebo_wind_plugin.h"
...
*/
	...
	if (_sdf->HasElement("windSpeedVariance"))
		wind_speed_variance_ = _sdf->Get<double>("windSpeedVariance");
	else
  ...
  wind_speed_pub_ = nh.advertise<geometry_msgs::Vector3>(wind_speed_pub_topic_, 1);
...
void GazeboWindPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  if (!use_custom_static_wind_field_) {
    ...
    // Fill in the wrench message
    wrench_stamped_msg.header.frame_id = frame_id_;
    wrench_stamped_msg.header.stamp = ros::Time::now();
    wrench_stamped_msg.wrench.force.x = wind.X() + wind_gust.X();
    wrench_stamped_msg.wrench.force.y = wind.Y() + wind_gust.Y();
    wrench_stamped_msg.wrench.force.z = wind.Z() + wind_gust.Z();
    wrench_stamped_msg.wrench.torque.x = 0;
    wrench_stamped_msg.wrench.torque.y = 0;
    wrench_stamped_msg.wrench.torque.z = 0;

    // Publish the wrench message
    wind_force_pub_.publish(wrench_stamped_msg);

    // Fill in the wind speed message
    wind_speed_msg.x = wind_velocity.X();
    wind_speed_msg.y = wind_velocity.Y();
    wind_speed_msg.z = wind_velocity.Z();

    // Publish the wind speed message
