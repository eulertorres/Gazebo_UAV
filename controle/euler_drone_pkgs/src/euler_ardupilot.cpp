...
#include "euler_plugins/euler_ardupilot.h"
...
GZ_REGISTER_MODEL_PLUGIN(EulerArduPilotPlugin)
...
// Private data class
class gazebo::EulerArduPilotPluginPrivate
{
  ...
  /// \brief ROS publisher for PWM outputs
  public: ros::Publisher rcOutPub;

  /// \brief Last time the RC outputs were published
  public: ros::Time lastRcOutPubTime;

  /// \brief Desired publication rate for RC outputs
  public: double rcOutPubRate = 40.0; // 40 Hz
...
/////////////////////////////////////////////////
EulerArduPilotPlugin::EulerArduPilotPlugin()
  : dataPtr(new EulerArduPilotPluginPrivate)
{
  this->dataPtr->arduPilotOnline = false;
  this->dataPtr->connectionTimeoutCount = 0;
  this->dataPtr->rosNode = nullptr;
}
...
EulerArduPilotPlugin::~EulerArduPilotPlugin()
{
  ...
  {
    delete this->dataPtr->rosNode;
    this->dataPtr->rosNode = nullptr;
  }
}

/////////////////////////////////////////////////
void EulerArduPilotPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  GZ_ASSERT(_model, "EulerArduPilotPlugin _model pointer is null");
  GZ_ASSERT(_sdf, "EulerArduPilotPlugin _sdf pointer is null");
...
*/
  ...
  if (!ros::isInitialized())
  {
    int argc = 0;
    char **argv = NULL;
    ros::init(argc, argv, "euler_ardupilot_plugin",
        ros::init_options::NoSigintHandler);
  }

  // Create ROS node handle
  this->dataPtr->rosNode = new ros::NodeHandle("");

  // Set up the publisher for RC outputs
  this->dataPtr->rcOutPub = this->dataPtr->rosNode->advertise<mavros_msgs::RCOut>("rc/out", 10);

  // Initialize the last publication time
  this->dataPtr->lastRcOutPubTime = ros::Time::now();

  // Listen to the update event. This event is broadcast every simulation
  // iteration.
  this->dataPtr->updateConnection = event::Events::ConnectWorldUpdateBegin(
      std::bind(&EulerArduPilotPlugin::OnUpdate, this));
...
/////////////////////////////////////////////////
void EulerArduPilotPlugin::OnUpdate()
{
  ...
  // Publish RC outputs at 40Hz
  ros::Time currentTime = ros::Time::now();
  double dt = (currentTime - this->dataPtr->lastRcOutPubTime).toSec();
  if (dt >= 1.0 / this->dataPtr->rcOutPubRate)
  {
    this->PublishRcOut();
    this->dataPtr->lastRcOutPubTime = currentTime;
  }

  this->dataPtr->lastControllerUpdateTime = curTime;
}

void EulerArduPilotPlugin::PublishRcOut()
{
  mavros_msgs::RCOut rcOutMsg;

  // Fill the header
  rcOutMsg.header.stamp = ros::Time::now();
  rcOutMsg.header.frame_id = "";

  // Fill the channels
  for (size_t i = 0; i < this->dataPtr->controls.size(); ++i)
  {
    double cmd = this->dataPtr->controls[i].cmd;

	//std::cout << "[euler_ardupilot_plugin] ---------------------------\n";

    // Clamp cmd to -1.0 to 1.0
    cmd = std::max(std::min(cmd, 1.0), -1.0);

    // Map cmd from -1.0 to 1.0 to PWM from 1000 to 2000
    uint16_t pwm = static_cast<uint16_t>(cmd*1000 + 1000.0);

	//std::cout << "[euler_ardupilot_plugin] cmd dps do clamp = " << cmd << " pwm calculado = " << pwm << " \n";

    // Clamp between 1000 and 2000
    pwm = std::max(std::min(pwm, static_cast<uint16_t>(2000)), static_cast<uint16_t>(1000));

	//std::cout << "[euler_ardupilot_plugin] pwm final = " << pwm << ".\n";

    rcOutMsg.channels.push_back(pwm);
  }

  // Ensure we have 16 channels by filling the rest with zeros if necessary
  while (rcOutMsg.channels.size() < 16)
  {
    rcOutMsg.channels.push_back(0);
  }

  // Publish the message
  this->dataPtr->rcOutPub.publish(rcOutMsg);
}

/////////////////////////////////////////////////
void EulerArduPilotPlugin::ResetPIDs()
{
...
/////////////////////////////////////////////////
bool EulerArduPilotPlugin::InitArduPilotSockets(sdf::ElementPtr _sdf) const
{
...
/////////////////////////////////////////////////
void EulerArduPilotPlugin::ApplyMotorForces(const double _dt)
{
...
/////////////////////////////////////////////////
void EulerArduPilotPlugin::ReceiveMotorCommand()
{
...
/////////////////////////////////////////////////
void EulerArduPilotPlugin::SendState() const
{
  // send_fdm
  fdmPacket pkt;
